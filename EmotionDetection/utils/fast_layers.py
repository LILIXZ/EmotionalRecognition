...
def conv_forward_strides_bk(x, w, b, conv_param):
    N, C, H, W = x.shape
    F, _, HH, WW = w.shape
    stride, pad = conv_param["stride"], conv_param["pad"]
    ...
    # Pad the input - only height and weight are padded, batch and channel and remaining unchanged
    p = pad
...
def conv_forward_strides(x, w, b, conv_param, mode):
    N, C, H, W = x.shape
    F, _, HH, WW = w.shape
    stride, pad = conv_param["stride"], conv_param["pad"]
