...
def softmax_loss_naive(W, X, y, reg):
    ...
    for i in range(num_trains):
      temp = scores[i] - np.max(scores[i])
      softmax = np.exp(temp) / np.sum(np.exp(temp))
...
def softmax_loss_vectorized(W, X, y, reg):
    ...
    loss = np.sum(-np.log(softmax_matrix[np.arange(num_trains), y]))
    
    softmax_matrix[np.arange(num_trains), y] -= 1
